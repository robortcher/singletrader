...
import os
from singletrader.datautils.dataapi.sqlapi import price_post_pg
...
def download_local(symbol_list=None,start_date=None,end_date=None,**kwargs):
    data = price_post_pg.query(start_date=start_date,end_date=end_date,universe=symbol_list)
    data = data.reset_index().rename(columns={'code':'symbol'})
    return data    
