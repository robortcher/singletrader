...
if __name__ == '__main__':
    ...
    q=query(finance.STK_MANAGEMENT_INFO).filter(finance.STK_MANAGEMENT_INFO.code=='600104.XSHG').order_by(finance.STK_MANAGEMENT_INFO.pub_date)
    df=finance.run_query(q)

    start_date = '2023-02-10'
    end_date = '2023-02-14'

    # pg_pm = Postgres(PricePostMinuteConfigPG)
    # down_price_minute(pg=pg_pm,start_date='2023-02-22',end_date='2023-03-01')

    # pg_au = Postgres(conf=AuctionConfigPG)
    # auw =  DataRetrive(start_date=start_date,end_date=end_date,mode='split',func=download_auction)
    # auw.datawriter(pg =pg_au)

    # print('====')
    
    # dw = DataRetrive(start_date=start_date,end_date=end_date,mode='split',func=get_valuation)
    # p = Postgres(conf=ValuationConfigPG)
    # p.close_connect()
    # dw.datawriter(pg=p)
    # d = down_price(trade_date='2023-02-06')
    # d2 = get_valuation(trade_date='2023-02-06')
    
    # pg_w= Postgres(conf=PricePostConfigPG)
    # pw = DataRetrive(start_date=start_date,end_date=end_date,mode='split',func=down_price)
    # pw.datawriter(pg=pg_w)
