...
class MultiFactorTesting():
    ...
    def __init__(self,bar_data, factor_data,price_type='close',add_shift=1,universe=None,market_cap_col='market_cap',freq=12):
        """
        Parameters
        -----------   
        bar_data :  pd.DataFrame,pd.MultiIndex(date,asset) 
            - 行情数据（需要包括市值信息）
            
        factor_data : pd.DataFrame,pd.MultiIndex(date,asset)
            - 因子数据
        price_type : str
            - 指定计算收益的价格
        add_shift : int
            - 因子使用滞后期，如果因子2023-01-04号计算，默认add_shift=1表示2023-01-05日开始使用该信号进行交易
              在计算forward_return的时候也是使用2023-01-05~2023-01-06的跨日收益
        market_cap_col : str
            - 指定用于表示市值的字段，用于cap weighted的benchmark收益计算
        freq : int
            - 所用数据的周期，12表示月度，1表示年度，250表示日度

        """
        self.bar_data = bar_data
        self.factor_data = factor_data
        if universe is not None:
            self.factor_data = self.factor_data[universe>0]
    ...
    def compute_forward_returns(self,**kwargs):
        ...
        price_type = kwargs.get('price_type',self.price_type)
        add_shift = kwargs.get('add_shift', self.add_shift)
        # universe = kwargs.get('unvierse',None)
        ...
        if self.universe is not None:
            forward_returns = forward_returns[self.universe.reindex(forward_returns.index)>0]
        return forward_returns
