...
import pandas as pd
import numpy as np
def performance_indicator(nvs:pd.DataFrame, riskless_ret=0.02,freq=252,language='cn',mkt_return=None):
    ...
    def __performance_indicator(nv:pd.Series, riskless_ret):
        ...
        ret_ann = portfolio_ret_daily.mean()*freq#(abs_ret+1) ** (freq / (len(nv_df)-1)) - 1
